...
export default function VerifyOtpScreen() {
  ...
  const handleVerify = async () => {
    ...
    if (!currentUserId) {
      Alert.alert("Error", "No se encontró la sesión. Por favor vuelve a intentar.");
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      inputRefs.current[0]?.focus();
      // Show confirmation
      Alert.alert("Código reenviado", "Te enviamos un nuevo código de verificación.");
    } catch (err) {
      const message =
        err instanceof Error ? err.message : "Error al reenviar el código. Intenta de nuevo.";
