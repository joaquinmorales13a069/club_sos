...
// ─── Types ─────────────────────────────────────────────────
type ParentescoType = "empleado" | "conyuge" | "hijo";
...
export default function VerifyAccountTypeScreen() {
  ...
  const [selectedType, setSelectedType] =
    useState<ParentescoType>("empleado");
  ...
  const handleContinue = async () => {
    try {
      ...
      if (selectedType === "empleado") {
         ...
         * with parentesco = "empleado".
         */
        router.push("/onboarding/member-info" as never);
      } else {
        /**
         * TODO: /onboarding/link-employee — Future screen that will:
         *   • Search for an existing miembro with parentesco = "empleado"
         *   • Request nombre_completo + documento_identidad of the titular
         *   • Store the resulting titular miembro $id as:
         *       miembros.titular_miembro_id
         * This links the dependent (cónyuge / hijo) to their titular employee.
         */
        router.push("/onboarding/link-employee" as never);
      }
    } catch (error) {
      console.error(
    ...
    <SafeAreaView className="flex-1 bg-sos-white dark:bg-[#101822]">
      ...
      <View className="flex-1 px-6 pt-2 pb-8">
        ...
        <Text className="mb-8 font-sans text-base leading-relaxed text-sos-gray dark:text-gray-400">
          Selecciona tu relación con la empresa para configurar tu perfil
          correctamente.
        </Text>
      ...
      >
        <Pressable
          ...
          className="flex-row justify-center items-center w-full h-14 rounded-full bg-sos-red active:opacity-90"
          style={{
